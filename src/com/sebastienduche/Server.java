            ...
            for (int i = 0; i < size; i++) {
                ...
                if (fileType.isForLibDirectory()) {
                    ...
                    if (libFile.exists()) {
                        ...
                        if (localMd5.equals(serverMd5)) {
                            debug("Skipping downloading file: " + name + " checksum OK");
                            continue;
                        } else {
                            debug("Need to download file: " + name + " server checksum: " + serverMd5 + " local checksum: " + localMd5);
                        }
                    }
                }
                downloadError = false;
                ...
                if (!serverMd5.isEmpty() && !file.isDirectory()) {
                    ...
                    try (InputStream stream = new FileInputStream(file)) {
                        ...
                        if (localMd5.equals(serverMd5)) {
                            debug(name + " checksum OK");
                        } else {
                            debug(name + " server checksum: " + serverMd5 + " local checksum: " + localMd5);
                            downloadError = true;
                        }
                    }
                    if (fileSize == 0) {
                        debug("Error: file size is null");
                        downloadError = true;
                    }
                }
                if (downloadError) {
                    debug("Error with file: " + name);
                    file.deleteOnExit();
                }
            }
        } catch (IOException e) {
